...
/* ===Страницы=== */
function groups(){
    $query = "SELECT group_id, group_nam FROM category_group ";
    $res = mysql_query($query);
...
function get_all_news($start_pos, $perpage){
    $query = "SELECT news_id, title, anons, date, type_nam FROM news INNER JOIN news_type ON news_type.type_id = news.type_id ORDER BY date DESC LIMIT $start_pos, $perpage";
    $res = mysql_query($query);
...
function add_news(){
    ...
    if(empty($title)){
        ...
        $query = "INSERT INTO news (title, keywords, description, date, anons, text, type_id)
                    VALUES ('$title', '$keywords', '$description', '$date', '$anons', '$text', $type_id)";
        $res = mysql_query($query);
...
/* ===Отдельная новость=== */
/* ===Типы новостей список без ограничений=== */
...
/* ===Тип новости=== */
/* ===Группы категорий список без ограничений=== */
function get_category_group(){
    $query = "SELECT * FROM category_group";
    $res = mysql_query($query);
...
function get_categorys_by_parent($group_id, $parent_id){
...
function get_parent_category_by_cat($cat_id){
    $query = "SELECT parent_id FROM category WHERE cat_id = $cat_id";
    $parent_id=0;
	$res = mysql_query($query);
	if(mysql_affected_rows() > 0){
...
function add_category(){
    ...
    $tabl_nam = clear_admin(trim($_POST['tabl_nam']));
 	$row_id = (int)$_POST['row_id'];
   
    if(empty($cat_name)){
        $_SESSION['add_category']['res'] = "<div class='error'>Вы не указали название категории</div>";
        return false;
    }else{
        // проверяем нет ли такой категории на одном уровне
        $query = "SELECT cat_id FROM category WHERE cat_name = '$cat_name' AND parent_id = $parent_id";
        $res = mysql_query($query);
        if(mysql_num_rows($res) > 0){
            $_SESSION['add_category']['res'] = "<div class='error'>Категория с таким названием уже есть</div>";
            return false;
        }else{
            $query = "INSERT INTO category (cat_name, parent_id, group_id, tabl_nam, row_id)
                        VALUES ('$cat_name', $parent_id, $group_id, '$tabl_nam', $row_id)";
            $res = mysql_query($query);
            if(mysql_affected_rows() > 0){
                $_SESSION['answer'] = "<div class='success'>Категория добавлена!</div>";
                return true;
            }else{
                $_SESSION['add_category']['res'] = "<div class='error'>Ошибка при добавлении категории!</div>";
                return false;
            }                        
        }
    }
...
function edit_category($cat_id){
    ...
    if(empty($cat_name)){
        $_SESSION['edit_category']['res'] = "<div class='error'>Вы не указали название категории</div>";
        return false;
    }else{
...
/*        $query = "SELECT cat_id FROM category WHERE cat_name = '$cat_name' AND parent_id = $parent_id";
        $res = mysql_query($query);
        if(mysql_num_rows($res) > 0){
            $_SESSION['edit_category']['res'] = "<div class='error'>Категория с таким названием уже есть</div>";
            return false;
        }else{*/
            ...
            if( strlen(mysql_error())==0){
                ...
                $_SESSION['edit_category']['res'] = "<div class='error'>Ошибка при редактировании категории!<br>".mysql_error()."<br>".$query."</div>";
                return false;
            }
       /* }*/
    }
...
function del_category($cat_id){
    $query = "SELECT COUNT(*) FROM category WHERE parent_id = $cat_id";
    $res = mysql_query($query);
    ...
    if($row[0]){
        ...
        mysql_query("DELETE FROM goods WHERE goods_catid = $cat_id");
...
function edit_product($id){
    ...
    $keywords = trim($_POST['keywords']);
    $description = trim($_POST['description']);
    $goods_catid = (int)$_POST['category'];
    $anons = trim($_POST['anons']);
...
function add_product(){
    ...
    if(empty($name)){
        ...
        $query = "INSERT INTO goods (name, keywords, description, goods_catid, anons, content, hits, new, sale, price, date, visible)
                    VALUES ('$name', '$keywords', '$description', $goods_catid, '$anons', '$content', '$hits', '$new', '$sale', $price, '$date', '$visible')";        
        $res = mysql_query($query) or die(mysql_error());
...
function add_file(){
    ...
    $file_comment = trim($_POST['file_comment']);
	$types = array("image/gif", "image/png", "image/jpeg", "image/pjpeg", "image/x-png"); // массив допустимых расширений
	if($_FILES['name_file']['name']){
		$name_fileExt = strtolower(preg_replace("#.+\.([a-z]+)$#i", "$1", $_FILES['name_file']['name'])); // расширение картинки
		//$name_fileName = "{$id}.{$name_fileExt}"; // новое имя картинки
		$name_fileTmpName = $_FILES['name_file']['tmp_name']; // временное имя файла
		$name_fileName  = time().'_'.rand(0,1000).'.'.$name_fileExt;
		$name_fileSize = $_FILES['name_file']['size']; // вес файла
		$name_fileType = $_FILES['name_file']['type']; // тип файла
		$name_fileError = $_FILES['name_file']['error']; // 0 - OK, иначе - ошибка
		$error = "";
		
		if(!in_array($name_fileType, $types)) $error .= "Допустимые расширения - .gif, .jpg, .png <br />";
		if($name_fileSize > SIZE) $error .= "Максимальный вес файла - 1 Мб";
		if($name_fileError) $error .= "Ошибка при загрузке файла. Возможно, файл слишком большой";
		
		if(!empty($error)) $_SESSION['answer'] = "<div class='error'>Ошибка при загрузке картинки товара! <br /> {$error}</div>";
		
		// если нет ошибок
		if(empty($error)){
			if(@move_uploaded_file($name_fileTmpName, "../userfiles/galerys/$name_fileName")){
				resize("../userfiles/galerys/$name_fileName", "../userfiles/galerys/thumb/$name_fileName", WMAX, HMAX, $name_fileExt);
				//@unlink("../userfiles/tmp/$name_fileName");
				$sql_text="INSERT INTO files (galery_id, file_nam, file_comment, path, path_small, type, size, pk_user) 
						VALUES ($galery_id, '".$name_fileName."', 
						'$file_comment', '', '','".$name_fileExt."', 
						".round($name_fileSize/1024,1).", 1)";
				$result = mysql_query($sql_text);
				if(!$result) {
					$_SESSION['error']['res'] = "<div class='error'>".mysql_error()."</div>";
					return false;
				} else {
					return true;
				}
			}else{
				$_SESSION['error']['res'] = "<div class='error'>Ошибка при добавлении товара</div>";
				return false;
			}
		}
	}
...
function del_file($file_id){
	...
	$query = "SELECT file_nam FROM files WHERE file_id = $file_id";
    $res = mysql_query($query);
	if(mysql_affected_rows() > 0){
		$item = array();
		$item = mysql_fetch_assoc($res);
		//Удалям маленькую и большую картинку, с подавлением ошибок.
		$p_link = str_replace (PATH, "", GALERYS);
		@unlink("../".$p_link."/".$item['file_nam']);
		@unlink("../".$p_link."/thumb/".$item['file_nam']);	
		...
		if(mysql_affected_rows() > 0){
			...
			$_SESSION['answer'] = "<div class='error'>Ошибка удаления!<br>".mysql_error()."</div>";
		}
	}else{
		$_SESSION['answer'] = "<div class='error'>Ошибка удаления!<br>".mysql_error()."</div>";
	}
